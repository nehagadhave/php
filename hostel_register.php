...
<div class="container">
		<div class="register">
		<p1><center>HOSTEL REGISTRATION</center></p1>		
...
$email=$_POST['email'];
	...
	$query="select licence from hostel_reg where licence='$licence'";
	$ans=$con->query($query);
 ...
 $url = "http://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($pin)."&sensor=false";
    ...
    $result = mysqli_query($con,$query);
        ...
        $query1="insert into hostel_reg values('','$name','$pin','$category','$licence','$contact','$email','$lat','$lng',0,1)";
	$query = $con->query($query1);

		
		echo "<script type='text/javascript'>"; 
        	echo "alert('You have registered successfully. Your Hostel will be available within few hours.')"; 
                echo "</script>"; 
...
function validateform_1() 
{
        ...
        var category1 = document.forms["register"]["category"].value;
